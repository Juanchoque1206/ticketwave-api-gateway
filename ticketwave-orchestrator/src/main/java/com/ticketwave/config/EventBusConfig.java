...
/**
 * RabbitMQ is the only bus transport for the orchestrator: events are consumed
 * from the shared {@code ticketwave.events} exchange and commands are published
 * to the {@code ticketwave.commands} exchange. Each service manages its OWN
 * queues (configured via {@code ticketwave.bus.*}), so every service receives
 * the messages it needs without competing consumers. Beans are only created
 * under the rabbitmq profile; tests provide in-memory doubles.
 */
@Configuration
public class EventBusConfig {
    ...
    public EventBus rabbitMqEventBus(RabbitTemplate rabbitTemplate, AmqpAdmin amqpAdmin,
                                     @Value("${ticketwave.bus.events-queue:ticketwave.events.orchestrator}") String eventsQueue) {
