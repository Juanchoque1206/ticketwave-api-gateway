 ...
 * <p>
 * It subscribes to domain events delivered by RabbitMQ, advances the saga
 * snapshot in the SagaStateRepository and drives each step by publishing
 * commands through the CommandBus. Every step transition is persisted before a
 * command is published so that a crash can be resumed from Redis via
 * {@link #recover()}.
 * <p>
